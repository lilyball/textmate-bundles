...
# Description:
#   Makes analyzing of a Grails path + filename easier.
# 
...
class GrailsPath
  ...
  def initialize(filepath = TextMate.filepath)
    if filepath[0..0] == '/'
      ...
      # Relative file, prepend grails_root
      @filepath = File.join(grails_root, filepath)
    end
  end
  ...
  # This is used in :file_type and :grails_path_for_view
  ...
  def grails_path_for(type)
    return grails_path_for_view if type == :view
    if TextMate.project_directory
      GrailsPath.new(File.join(grails_root, stubs[type], artifact_base_name_modified_for(type) + default_extension_for(type)))
    else
