# 'parses' the output of hg log and makes html out of
...
$limit         = ENV['TM_HG_LOG_LIMIT'].nil?   ? 9 : ENV['TM_HG_LOG_LIMIT'].to_i
...
begin
   ...
   $hg_url = `"${TM_HG:=hg}" info #{ENV['PWD'].quote_filename_for_shell}|grep URL|cut -b6-`.chop

   $stdin.each_line do |line|
      raise HGErrorException, line  if line =~ /^hg:/
         ...
         when :comment
            unless changed_files.empty?
               ...
               changed_files.each do |path|
                  ...
                  $base_url = $hg_url.split($file.slice(%r(/[^/]*)))[0]
