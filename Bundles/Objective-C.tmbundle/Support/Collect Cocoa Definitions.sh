...
# Cocoa Classes
find /System/Library/Frameworks/{AppKit,Foundation}.framework -name \*.h -exec grep '@interface NS[A-Za-z]*' '{}' \;|perl -pe 's/.*?(NS[A-Za-z]+).*/$1/'|sort|uniq|./list_to_regexp.rb >/tmp/classes.txt

# Cocoa Protocols
find /System/Library/Frameworks/{AppKit,Foundation}.framework -name \*.h -exec grep '@protocol NS[A-Za-z]*' '{}' \;|perl -pe 's/.*?(NS[A-Za-z]+).*/$1/'|sort|uniq|./list_to_regexp.rb >>/tmp/classes.txt
