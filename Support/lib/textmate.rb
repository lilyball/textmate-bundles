...
module TextMate
	...
	class ProjectFileFilter
		def initialize
			@file_pattern = load_pattern(:TM_FILE_PATTERN, load_pattern('OakFolderReferenceFilePattern', '!(/\.(?!htaccess)[^/]*|\.(tmproj|o|pyc)|/Icon\r)$'))
			@folder_pattern = load_pattern(:TM_FOLDER_PATTERN, load_pattern('OakFolderReferenceFolderPattern', '!.*/(\.[^/]*|CVS|_darcs|\{arch\}|blib|.*~\.nib|.*\.(framework|app|pbproj|pbxproj|xcode(proj)?|bundle))$'))
		...
		def load_pattern (key, default_pattern)
			case key
				when Symbol
					str = ENV[key.to_s] || ""
				when String
					str = %x{defaults 2>/dev/null read com.macromates.textmate #{key.to_s}}
			end
			str = str.gsub(/\\\\/, '\\').chomp
